...
let chance: number = 0;
let randNum: number = Math.floor(Math.random() * 100) + 1;
// console.log(randNum);

let guesedNum: { guesedNumber: number } = await inquirer.prompt([
    {
...
const initApp = async (): Promise<number> => {
    while (guesedNum.guesedNumber !== randNum) {
        ...
        chance++;
        if (
            guesedNum.guesedNumber === undefined ||
            ...
            console.log(
                `${chalk.yellow(`${`Your Guessed Number is too Low.`}\n`)}${chalk.blueBright(`${`Please Try Again!`}`)}`
            );
        } else if (guesedNum.guesedNumber > randNum) {
            console.log(
                `${chalk.yellow(`${`Your Guessed Number is too High.`}\n`)}${chalk.blueBright(`${`Please Try Again!`}`)}`
            );
        }
    ...
    console.log(
        `${chalk.green(`${`Congratulations! You Guessed it Right in ${chance + 1
            } tries`} \n ${`Your Score is ${score}`}`)}`
    );
